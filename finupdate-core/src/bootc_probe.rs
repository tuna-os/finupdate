...
//!
//! Moved here from `src/ui/bootc_probe.rs` (finupdate#111): this module has no
//! GTK dependency of its own — the two icon-theme lookups that did need a GDK
//! display stayed behind in `src/ui/bootc_probe.rs`, which now calls
//! [`resolve_logo_file`], [`read_logo_names`], and [`HeroLogo`] from here.
//! Everything else was already the same shape as the rest of this crate:
//! `std::process::Command` invocation of `bootc`, `serde_json` parsing, and
//! calls into [`crate::registry_client`] / [`crate::settings`].
//!
...
pub fn read_image_info() -> Option<String> {
...
pub fn read_os_release_field(key: &str) -> Option<String> {
...
pub fn read_booted_image_summary() -> Option<String> {
...
pub fn parse_booted_image_summary(json: &Value) -> Option<String> {
...
pub fn read_booted_tag_suffix() -> Option<String> {
    if let Some(mock) = Settings::load().mock_identity {
...
/// unit testing.
pub fn parse_booted_tag_suffix(json: &Value) -> Option<String> {
    let img = json
...
pub fn extract_yyyymmdd_date(s: &str) -> Option<chrono::NaiveDate> {
...
///
/// The enum itself carries no GTK dependency — only the caller that
/// constructs a `Themed` variant from `gtk::IconTheme::has_icon` does, which
/// is why it stays in `src/ui/bootc_probe.rs` alongside that lookup.
pub enum HeroLogo {
...
pub const LOGO_SEARCH_ROOTS: &[&str] = &[
...
pub fn resolve_logo_file<P: AsRef<Path>>(roots: &[P], logo: &str) -> Option<PathBuf> {
    if logo.is_empty() {
        return None;
    }
    for root in roots {
...
pub fn read_logo_names() -> Vec<String> {
...
pub static BOOTC_STATUS_CACHE: Mutex<Option<Value>> = Mutex::new(None);

pub fn get_cached_bootc_status() -> Option<Value> {
...
/// subprocess.
pub static BOOTC_IMAGE_INFO_CACHE: std::sync::OnceLock<Option<(String, String, String)>> =
    std::sync::OnceLock::new();

pub fn detect_bootc_image_info() -> Option<(String, String, String)> {
    BOOTC_IMAGE_INFO_CACHE
        .get_or_init(detect_bootc_image_info_uncached)
        .clone()
}

pub fn detect_bootc_image_info_uncached() -> Option<(String, String, String)> {
...
pub struct BootcImageInfoConfig {
    pub tags: Vec<String>,
}

pub fn read_bootc_image_info_config() -> Option<BootcImageInfoConfig> {
...
pub fn read_registry_uri() -> Option<String> {
    detect_bootc_image_info().map(|(_, registry_uri, _)| registry_uri)
}

pub fn read_selected_tag() -> String {
...
pub fn is_pinned_tag(tag: &str) -> bool {
...
pub fn parse_image_ref_fields(img_ref: &str) -> (String, String, String) {
    if img_ref.is_empty() {
        return (
            "Unknown".to_string(),
            "latest".to_string(),
...
/// Deployment representation for the collapsible version history list.
///
/// Moved here alongside the probe logic that builds it (finupdate#111): the
/// struct itself has no GTK dependency, only the widget-building code that
/// renders it (still in `src/ui/history_list.rs`) does.
#[derive(Debug, Clone)]
#[allow(dead_code)]
...
pub fn get_real_deployments_from_json(json: &Value) -> Option<Vec<MockDeployment>> {
    let mut ds = Vec::new();
...
pub fn get_real_deployments() -> Option<Vec<MockDeployment>> {
    get_cached_bootc_status().and_then(|json| get_real_deployments_from_json(&json))
}

pub fn get_host_kernel() -> String {
